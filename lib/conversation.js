...
Conversation.querySong = function(senderId, messageText) {
    ...
    spotifyApi.queryTrack(messageText, function(songData) {
        var item = songData.tracks.items[0];
        var messageTemplate = Conversation.generateSongResponseTemplate(sId, item);
        var audioTemplate = Conversation.generateSongPreviewMessage(sId, item);
...
Conversation.generateSongResponseTemplate = function(senderId, songData) {
    ...
    if (songData.artists.length > 1) {
        title = title + ` ft. ${songData.artists[1].name}`;
    }
...
Conversation.generateSongPreviewMessage = function(senderId, songData) {
    var audioPreview = {
        recipient: {
            id: senderId
        },
